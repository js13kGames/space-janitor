...
function main() {
    try {
        ...
        if (arrowKeyLeft) player.rotate(Math.PI / -15);
        if (arrowKeyRight) player.rotate(Math.PI / 15);
        ...
        blackholes.forEach((hole, index) => {
           hole.aliveTime = hole.aliveTime + Date.now() - lastUpdate;
            if (hole.aliveTime >= 5 * 1000){
                engine.deleteBody(hole);
                renderer.deleteBody(hole);
        ...
        if (bodies.length < 30) {
            ...
            if (output != null && output.position.distance(player.position) > 3 * player.h) {
        ...
        engine.collisions.forEach(function (collision) {
            ...
            if (collision[BodyType.player] && collision[BodyType.asteroid]) {
                const player = collision[BodyType.player];
                engine.deleteBody(player);
                renderer.deleteBody(player);
                const index = bodies.indexOf(player);
               
                document.querySelector("#hud").innerText = "Game over"
                document.querySelector("#hud").style.color = "white";
                document.querySelector("#hud").style.fontSize = "100px";
                document.querySelector("#hud").style.textAlign = "center";

                if (index > -1) bodies.splice(index, 1);
            }
            //Player, Trash collision 
            else if (collision[BodyType.player] && collision[BodyType.trash]) {
                const trash = collision[BodyType.trash];
                engine.deleteBody(trash);
                renderer.deleteBody(trash);
                const index = bodies.indexOf(trash);

                if (!trashPickedUp.includes(collision[BodyType.trash].id)) {
                    trashPickedUp.push(collision[BodyType.trash].id);
                    
                    document.querySelector("#score").innerText = trashPickedUp.length;
            ...
            //Blackhole collision
            else if (collision[BodyType.player] && collision[BodyType.blackhole]) {
                const player = collision[BodyType.player];
                engine.deleteBody(player);
                renderer.deleteBody(player);
                const index = bodies.indexOf(player);
            ...
            else if (collision[BodyType.trash] && collision[BodyType.blackhole]) {
                const trash = collision[BodyType.trash];
                engine.deleteBody(trash);
                renderer.deleteBody(trash);
                const index = bodies.indexOf(trash);
            ...
            else if (collision[BodyType.asteroid] && collision[BodyType.blackhole]) {
                const asteroid = collision[BodyType.asteroid];
                engine.deleteBody(asteroid);
                renderer.deleteBody(asteroid);
                const index = bodies.indexOf(asteroid);
                if (index > -1) bodies.splice(index, 1);
            }

        })
        
        blackHoleTimer = blackHoleTimer + Date.now() - lastUpdate;
        
        
        
        if (blackHoleTimer >= 10 * 1000){
            ...
            if (blackHole.position.distance(player.position) > 5 * player.h){
