...
function main() {
    try {
        ...
        if (arrowKeyUp) engine.applyForce(player, Vector.from(player.direction).multiply(20));
        ...
        engine.collisions.forEach(function(collision){
            if (collision.a.type == BodyType.player && collision.b.type == BodyType.trash) {
                engine.deleteBody(collision.b)
                renderer.deleteBody(collision.b)
                bodies.splice(bodies.indexOf(collision.b), 1)
            }
           else if (collision.b.type == BodyType.player && collision.a.type == BodyType.trash) {
                engine.deleteBody(collision.a)
                renderer.deleteBody(collision.a)
                bodies.splice(bodies.indexOf(collision.a), 1)
            }
