export class Vector {
     ...
     */
    update(x, y) {
        if ('x' in x) Object.assign(this, x);
        else {
            this.x = x;
            this.y = y;
        }
...
export class Body {
    ...
    calculateExtents() {
        ...
        this.extents = [
            Vector.create(l * Math.cos(rotation + Math.PI / 4), l * Math.sin(rotation + Math.PI / 4)),
            Vector.create(l * Math.cos(rotation + 3 * Math.PI / 4), l * Math.sin(rotation + 3 * Math.PI / 4)),
            Vector.create(l * Math.cos(rotation + 5 * Math.PI / 4), l * Math.sin(rotation + 5 * Math.PI / 4)),
            Vector.create(l * Math.cos(rotation + 7 * Math.PI / 4), l * Math.sin(rotation + 7 * Math.PI / 4))
        ];
    }
}

export default class Physics {
    bodyTypes = Object.freeze({
    ...
    addBody(body) {
        ...
        body.grid.update(Vector.from(body.position).divide(this.gridSize).ceil());
    ...
    updateCollisionArea(body) {
        ...
        else if (nextGrid.x > this.gridSize.x) nextGrid.x = this.gridSize.x - nextGrid.x;
        if (nextGrid.y < 1) nextGrid.add({ y: nextGrid.y, x: 0 });
        else if (nextGrid.y > this.gridSize.y) nextGrid.y = this.gridSize.y - nextGrid.y;
    ...
    update(time) {
        ...
        this.bodies.forEach(b => {
            ...
            potentialColliders.forEach((collider) => {
                const { extents: [extent], position } = collider;
                const d = position.distance(extent);
                for (const extent of b.extents) {
                    if (extent.distance(position) < d) {
